    ...
    pub async fn refresh_handler(
        ...
        loop {
            let sleep_dur = match ca_client.fetch_certificate(&id).await {
                ...
                Ok(fetched_certs) => {
                    info!(
                        "certificate to be refresed called"
                    );
