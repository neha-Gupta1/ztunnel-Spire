...
// certificates are create for CertificateProvider
// at many places like-
// CAClient
// sercret manager 
#[async_trait]
...
// Neha - 
        info!(
            "fetch_certificate called for CAClient"
        );

        warn!("fetching certificate: {}", id);
        if check_socket_path(){
            info!(
                "it could find the path"
            );
        }
        // Neha - end
...
// check_socket_path const security.WorkloadIdentitySocketPath untyped string = "./var/run/secrets/workload-spiffe-uds/socket"

fn check_socket_path() -> bool{
    let path=String::from("/run/spire/sockets/agent.sock");
   
  if Path::new(&path).exists(){
    println!("path found function.");
    true
  } else{
    println!("path not found function.");
    false
  }
    // assert_eq( Path::new(&path).is_file(), false)
